...
class KanbanStore {
  ...
  _seedDefault() {
    ...
    const boardId = 'main';
    this.s.insertBoard.run(boardId, 'Main Board', 'Primary task tracking board');
    for (const col of DEFAULT_COLUMNS) {
      this.s.insertCol.run(col.id, boardId, col.title, col.order, col.wip_limit);
    }

    // Seed some example cards
    const examples = [
      { col: 'backlog', title: 'Set up monitoring dashboards', desc: 'Configure Grafana + Prometheus for all infrastructure', priority: 'normal' },
      { col: 'backlog', title: 'Document API endpoints', desc: 'Create OpenAPI spec for Mission Control REST API', priority: 'low' },
      { col: 'queued', title: 'Configure WAF rules', desc: 'Set up Cloudflare WAF rules for production endpoints', priority: 'high' },
      { col: 'in_progress', title: 'Email triage automation', desc: 'Build workplan for automated email categorization', priority: 'normal', agent: 'demo-nova' },
      { col: 'review', title: 'PR #412 code review', desc: 'Review authentication refactor pull request', priority: 'high', agent: 'demo-atlas' },
      { col: 'done', title: 'SSL certificate renewal', desc: 'Renewed production TLS certs', priority: 'normal' },
    ];
