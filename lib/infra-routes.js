...
function registerInfraRoutes(pathname, req, res, registry, auth) {
  ...
  if (pathname === '/api/infra/types' && req.method === 'GET') {
    return json(res, 200, {
      componentTypes: require('./lib/infrastructure-registry').InfrastructureRegistry.COMPONENT_TYPES,
      accessLevels: require('./lib/infrastructure-registry').InfrastructureRegistry.ACCESS_LEVELS,
    });
  }
